...
--The results are not the same because when specifying the predicate in the ON clause will make that join take into consideration ONLY the values with that value for that JOIN operation while specifying predicate on WHERE will filter the final results after all the information are joined together
