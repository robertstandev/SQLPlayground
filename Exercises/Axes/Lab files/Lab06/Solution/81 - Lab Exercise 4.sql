...
SELECT
	...
	,RIGHT(contactname, CHARINDEX(',', REVERSE(contactname)) - 1)
FROM Sales.Customers
...
SELECT
	custid
	,CONCAT('C', REPLICATE('0', 5 - LEN(custid)), custid)
FROM Sales.Customers
...
SELECT
	contactname
	,LEN(contactname) - LEN(REPLACE(contactname,'a','')) AS NrOfAs
FROM Sales.Customers
ORDER BY NrOfAs DESC
